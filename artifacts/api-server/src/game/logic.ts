...
function startMiniGame(taskId: string, defKey: string, type: MiniGameType): void {
  ...
  gs.activeMiniGame = {
    ...
    sequence: [
      ...
      Math.floor(Math.random() * 9) + 1,
      Math.floor(Math.random() * 9) + 1,
    ].sort(defKey === 'check_meter' ? (a, b) => a - b : () => 0),
    seqIndex: 0, seqWrong: false,
    dialAngle: 0, dialTarget: Math.floor(Math.random() * 360),
    dialGreenWidth: 22, dialStops: 0, dialRequiredStops: defKey === 'close_tap' ? 2 : 3,
    letterText,
...
// ─── Multiplayer tick ─────────────────────────────────────────────────────────
// Runs one full game tick for multiple human players.
// Each player's input is applied before the shared world simulation.

// No-input sentinel used when running shared systems without a specific player's context.
const NOOP_INPUT: import('./types').InputState = {
  dx: 0, dy: 0,
...
export function tickGameMulti(dt: number, allInputs: Map<string, import('./types').InputState>): void {
  if (gs.phase === 'briefing') {
    gs.briefingTimer = Math.max(0, gs.briefingTimer - dt);
    if (gs.briefingTimer <= 0) gs.phase = 'play';
    return;
  ...
  if (gs.phase === 'play') {
    ...
    // Phase 1 — movement: safe to run per-player (no shared timer side-effects)
    for (const [playerId, input] of allInputs) {
      if (gs.phase !== 'play') break;
      gs.localPlayerId = playerId;
      updateHumanPlayer(dt, input);
    }

    // Phase 2 — interactions: per-player edge-triggered events (body reports,
    // meeting calls, sabotage fixes, siphon start). Each player independently
    // presses E; the phase-guard inside updateInteractions handles double-fires.
    for (const [playerId, input] of allInputs) {
      if (gs.phase !== 'play') break;
      gs.localPlayerId = playerId;
      updateInteractions(dt, input);
    ...
    // Phase 3 — shared world simulation (runs ONCE per tick regardless of player
    // count). updateMiniGame and updateSabotages advance timers by dt; running
    // them per-player would cause N× speed-up. Mini-game tap/digit events are
    // handled via room.handleAction → onMiniGameTap/onMiniGameDigitTap instead.
    updateMiniGame(dt, NOOP_INPUT);
    updateSabotages(dt, NOOP_INPUT);
    updateBots(dt);
    updateSiphoning(dt);
    updateImmunity(dt);
    updatePrompt(dt);
